...
BASIC LEGACY_A "Legacy basic event" layer=Legacy`;
  ...
  basic X2 {
    ...
    assert(await evaluate(`document.querySelector("#fault-tree-analysis").textContent.includes("Qualitative result") && document.querySelector("#fault-tree-analysis").textContent.includes("Review priorities")`), "qualitative fault-tree screening results are missing");
    await fill("#fault-tree-search", "Sensor one");
    assert(await evaluate(`document.querySelector("#fault-tree-canvas").textContent.includes("Sensor one dangerous failure") && document.querySelector("#fault-tree-canvas").querySelectorAll(".search-match").length === 1`), "fault tree entity search did not filter and highlight the matching event");
    await fill("#fault-tree-search", "");
    await fill("#fault-tree-cutset-limit", "10");
    assert(await evaluate(`document.querySelector("#fault-tree-cutset-limit").value`) === "10", "cut-set visible-row control did not update");
