...
Use **Fault tree analysis** to model top events deductively with a structured domain-specific language. The editor supports nested and layered diagrams, architecture-linked basic events, architecture-generated starter trees, top-down connector layout, zoom controls, and continuous DSL validation. The builder provides `AND`, `OR`, `NAND`, `NOR`, `XOR`, and `NOT`; existing DSL files with `KOFN:k/n` remain readable.
