...
var itemBy = (group, id) => state[group].find((item) => item.id === id);
...
function named(group, id) {
  return itemBy(group, id)?.name || "Not linked";
}
...
function validateFaultTreeGate(id, gate, children, warnings) {
  ...
  if (gate !== "NOT" && !gate.startsWith("KOFN:") && count === 1)
    warnings.push(`Gate "${id}" has one child; remove the gate or add the intended independent inputs.`);
  ...
  if (count !== n)
    throw new Error(`K-of-N gate "${id}" declares N=${n} but has ${count} children.`);
  if (k === 1)
    warnings.push(`K-of-N gate "${id}" is 1-of-${n}; use OR unless voting terminology is required.`);
  if (k === n)
    warnings.push(`K-of-N gate "${id}" is ${n}-of-${n}; use AND unless voting terminology is required.`);
}
...
function componentsFromPlantUml(source) {
  ...
  for (const match of String(source || "").matchAll(pattern)) {
    ...
    const id = match[3] || name.replace(/\W+/g, "_").toUpperCase();
...
${components.map((component) => {
    ...
    const children = modes.map(([suffix]) => `${id}_${suffix}`);
...
function approvedEvidence(id) {
  return state.assurance.evidence.some((item) => item.id === id && item.status === "Approved");
}
...
function assuranceFieldHtml(field, record) {
  ...
  if (kind === "textarea")
    ...
    control = `<select name="${name}">${values.map((option) => `<option ${option === value ? "selected" : ""}>${esc(option)}</option>`).join("")}</select>`;
    ...
    control = `<input type="${kind === "date" ? "date" : "text"}" name="${name}" value="${esc(value)}" placeholder="${esc(values || "")}" />`;
...
function renderFaultTree() {
  ...
  try {
    let getGateSvg2 = function(gate) {
    ...
    }, branchFor2 = function(id, depth = 0) {
      if (!visible(id))
        return null;
      const node = model.nodes.get(id);
      return { id, depth, children: node.children.map((child) => branchFor2(child, depth + 1)).filter(Boolean), x: 0, y: 0 };
    }, place2 = function(branch) {
      maxDepth = Math.max(maxDepth, branch.depth);
      if (!branch.children.length) {
        branch.x = cursor;
        cursor += 250;
      } else {
        branch.children.forEach(place2);
        branch.x = (branch.children[0].x + branch.children[branch.children.length - 1].x) / 2;
      }
      branch.y = 34 + branch.depth * 176;
    }, drawConnectors2 = function(branch) {
      const paths = branch.children.map((child) => {
        const joinY = branch.y + 132 + Math.max(20, (child.y - branch.y - 132) / 2);
        return `<path d="M ${branch.x} ${branch.y + 132} V ${joinY} H ${child.x} V ${child.y}"/>${drawConnectors2(child)}`;
      });
      return paths.join("");
    }, drawNode2 = function(branch) {
      ...
      return `<article class="fault-node ${node.kind} ${matchesNode(node) && query ? "search-match" : ""}" style="left:${branch.x}px;top:${branch.y}px">
        <div class="fault-node-card"><span class="fault-gate">${getGateSvg2(gate)}</span><span class="fault-gate-label">${esc(gate)}</span><strong>${esc(node.id)}</strong><p>${esc(node.label)}</p>${node.component ? `<small>Component: ${esc(node.component)} · ${esc(named("components", node.component))}</small>` : ""}<small>${esc(node.layer)}</small></div>
      </article>${branch.children.map(drawNode2).join("")}`;
    };
    ...
    if (!branches.length)
      canvas.innerHTML = `<p class="standards-note">No nodes are assigned to ${esc(state.faultTree.activeLayer)}. Add or move nodes with the builder.</p>`;
    else {
    ...
    selectedCuts.forEach((set) => {
      ...
      set.forEach((id) => {
        const item = participation.get(id) || { count: 0, score: 0 };
        item.count += 1;
        item.score += 1 / set.length;
        participation.set(id, item);
      });
    });
...
function renderFaultTreeBuilder() {
  ...
  function updateInputsOptions() {
    ...
    const options2 = (items) => items.map((item) => `<option value="${esc(item.id || item)}">${esc(item.id || item)}${item.label ? ` — ${esc(item.label)}` : ""}${item.layer ? ` [${esc(item.layer)}]` : ""}</option>`).join("");
    inputsSel.innerHTML = [
...
document.addEventListener("click", (event) => {
  ...
  if (removeAssurance) {
    ...
    else if (confirm(`Delete lifecycle record "${id}"?`)) {
      state.assurance[kind] = state.assurance[kind].filter((item) => item.id !== id);
      auditAssurance("Deleted", kind, id);
      save();
    }
  }
  ...
  if (removeRequirement) {
    ...
    if (state.assurance.tests.some((test) => test.requirement === id))
      alert("This requirement is referenced by V&V records and cannot be deleted.");
    else if (confirm(`Delete requirement "${id}"?`)) {
...
$("#catalog-form").addEventListener("submit", (event) => {
  ...
  try {
    id = requireUniqueIdentifier(state[group], requireIdentifier(data.id, "Identifier"), "Identifier");
...
["quant-function", "quant-target", "quant-mode", "quant-architecture"].forEach((id) => $(`#${id}`).addEventListener("change", (event) => {
  ...
  state.quantitative[keys[id]] = event.target.value;
  save();
}));
...
$("#render-btn").addEventListener("click", async () => {
  ...
  try {
    ...
    message.textContent = error.message || "Unable to render diagram. Start the app with bun server.js.";
