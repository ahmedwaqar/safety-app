...
function fillWorkflowActivityForm(activity = {}) {
  ...
  ["phaseId", "owner", "title", "objective", "inputs", "outputs", "safetyCheckpoint", "analysis", "status", "standardReference", "completionCriteria", "evidence"].forEach((key) => {
...
$("#add-requirement-btn").addEventListener("click", () => {
  ...
  form.reset();
  form.elements.hazard.innerHTML = options(state.hazards);
  form.elements.component.innerHTML = options(state.components);
...
$("#requirement-form").addEventListener("submit", (event) => {
  ...
  const data = formValues(event.target);
  try {
    data.id = requireUniqueIdentifier(state.requirements, requireIdentifier(data.id, "Requirement identifier"), "Requirement identifier");
